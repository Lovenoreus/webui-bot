...
# Pre-download ONNX model to avoid timeout during training (synchronous, as it's a one-time setup)
...
class VannaModelManager:
    """Manager class to handle Vanna with LLM providers for SQL generation (async support)"""
        ...
        if provider == "openai":
            class MyVannaOpenAI(ChromaDB_VectorStore, OpenAI_Chat):
                def __init__(self, config=None):
                    ...
                    # Initialize with AsyncOpenAI client for async support
                    client_config = config or {}
                    client_config['http_client'] = httpx.AsyncClient(timeout=60.0)  # Increased timeout
                    client_config['client'] = AsyncOpenAI(
                        api_key=client_config.get('api_key'),
                        http_client=client_config.get('http_client')
                    )
                    OpenAI_Chat.__init__(self, config=client_config)

                @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
                async def train(self, **kwargs):
                    return await super().train_async(**kwargs)
            ...
            class MyVannaOllama(ChromaDB_VectorStore, Ollama):
                def __init__(self, config=None):
                    ChromaDB_VectorStore.__init__(self, config=config)
                    # Configure Ollama with async HTTP client
                    client_config = config or {}
                    client_config['http_client'] = httpx.AsyncClient(timeout=60.0)
                    Ollama.__init__(self, config=client_config)
    ...
    async def initialize_vanna(self, provider: Optional[str] = None):
        """Initialize Vanna with specified provider or use config default (async)"""
        target_provider = provider or self.current_provider

        if target_provider == "openai":
            await self._init_openai_vanna()
        elif target_provider == "ollama":
            await self._init_ollama_vanna()
        else:
    ...
    async def _init_ollama_vanna(self):
        """Initialize Vanna with Ollama (async)"""
    ...
    async def generate_sql(self, query: str) -> str:
        ...
        try:
            sql = await self.vanna_client.generate_sql_async(query)
            if not sql:
...
async def main():
    ...
    DATABASE_MODE = os.getenv("DATABASE_MODE", "remote").lower()  # Default to remote
    ...
    if DATABASE_MODE == "remote":
        ...
        invoice_doc = """
        Sample data from Invoice table (first 5 rows):
        INVOICE_ID,ISSUE_DATE,SUPPLIER_PARTY_LEGAL_ENTITY_COMPANY_ID,SUPPLIER_PARTY_NAME,CUSTOMER_PARTY_NAME,DOCUMENT_CURRENCY_CODE,LEGAL_MONETARY_TOTAL_PAYABLE_AMOUNT
        INV001,2024-01-01,COMP001,JA Hotel Karlskrona,Region Västerbotten,SEK,12500.00
        INV002,2024-01-02,COMP001,JA Hotel Karlskrona,Stockholms Stad,SEK,9800.50
        INV003,2024-01-03,COMP002,Visma Draftit AB,Region Skåne,SEK,45000.75
        INV004,2024-01-04,COMP003,Abbott Scandinavia,Västra Götaland,SEK,32000.00
        INV005,2024-01-05,COMP004,Nordic IT Solutions AB,Region Västerbotten,SEK,15000.25
        """

        invoice_line_doc = """
        Sample data from Invoice_Line table (first 5 rows):
        INVOICE_ID,ISSUE_DATE,SUPPLIER_PARTY_LEGAL_ENTITY_COMPANY_ID,INVOICE_LINE_ID,ITEM_NAME,INVOICED_QUANTITY,PRICE_AMOUNT,INVOICED_LINE_EXTENSION_AMOUNT,ITEM_TAXCAT_PERCENT
        INV001,2024-01-01,COMP001,LINE001,Hotel Accommodation,2.0,5000.00,10000.00,25.0
        INV001,2024-01-01,COMP001,LINE002,Training Services,1.0,2500.00,2500.00,25.0
        INV002,2024-01-02,COMP001,LINE003,Hotel Accommodation,1.0,9800.50,9800.50,25.0
        INV003,2024-01-03,COMP002,LINE004,Software License,1.0,45000.75,45000.75,25.0
        INV004,2024-01-04,COMP003,LINE005,Medical Supplies,10.0,3200.00,32000.00,25.0
        """

    # Train Vanna with DDL and documentation
    print("\n📚 Training Vanna with DDL and documentation...")
    print("Training with Invoice DDL...")
    if await vanna_train(ddl=invoice_ddl):
        print("✅ Successfully trained Invoice DDL")
    else:
        print("❌ Failed to train Invoice DDL")

    print("Training with Invoice_Line DDL...")
    if await vanna_train(ddl=invoice_line_ddl):
    ...
    if await vanna_train(documentation=invoice_doc):
    ...
    if await vanna_train(documentation=invoice_line_doc):
